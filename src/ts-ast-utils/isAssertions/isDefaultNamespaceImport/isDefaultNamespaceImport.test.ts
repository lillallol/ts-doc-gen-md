...
import { isDefaultNamespaceImport } from "./isDefaultNamespaceImport";
...
describe(isDefaultNamespaceImport.name, () => {
    it(
        "0",
        unitTest({
            statementIndex: 0,
...
function unitTest(_: { statementIndex: number; toBe: boolean }): () => void {
    ...
    return (): void => {
        ...
        expect(isDefaultNamespaceImport(statement)).toBe(toBe);
    };
}
